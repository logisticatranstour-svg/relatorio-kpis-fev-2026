...
def is_refusal(x) -> bool:
    if pd.isna(x):
        return False
    s = str(x).strip().lower()
    return "recusou" in s or "recusa" in s
...
def find_patient_col(df: pd.DataFrame) -> str | None:
    # tenta achar coluna de nome do paciente/beneficiário sem quebrar se não existir
    candidates = []
    for c in df.columns:
        cl = str(c).strip().lower()
        if "paciente" in cl or "benefici" in cl or ("nome" in cl and ("paciente" in cl or "benefici" in cl)):
            candidates.append(c)
...
<title>Relatório Gerencial de KPIs</title>
...
<style>
  ...
  .logo{height:48px;max-width:220px;object-fit:contain;background:rgba(255,255,255,.06);padding:6px 10px;border-radius:10px}
...
<div class="wrap">
  <div class="phone-shell" id="phoneShell">
    <div class="topbar">
      <div class="toprow">
        <div class="logos">__LOGOS_HTML__</div>
        <div style="flex:1;min-width:260px;text-align:center">
          <h1>RELATÓRIO GERENCIAL DE KPIs</h1>
          <div class="sub">Atualizado em: <b>__GENERATED__</b></div>
    ...
    <div class="section charts">
      <div class="card">
        ...
        <div class="muted" id="satHint"></div>
      </div>
      <div class="card">
...
<script>
  ...
  const FIELDS = {
    itens: "__COL_ITENS__",
    uniforme: "__COL_UNIFORME__",
    produtos: "__COL_PRODUTOS__",
    atendimento: "__COL_ATENDIMENTO__",
    horario: "__COL_HORARIO__",
    nota: "__COL_NOTA__"
  };
  ...
  function calc(arr){
    ...
    const itens = countSimNao(arr, FIELDS.itens);
    const uniforme = countSimNao(arr, FIELDS.uniforme);
    const produtos = countSimNao(arr, FIELDS.produtos);
    const atendimento = countSimNao(arr, FIELDS.atendimento);
    const horario = countSimNao(arr, FIELDS.horario);

    const notas = arr.map(r => Number(r[FIELDS.nota])).filter(x => !Number.isNaN(x));
    const notasValidas = notas.length;

    const mean = notasValidas ? (notas.reduce((a,b)=>a+b,0)/notasValidas) : 0;
    const csat = notasValidas ? (mean/5)*100 : 0;
  ...
  function statusTxt(val, meta){
    return val >= meta ? "✓ Atingida" : "✗ Não Atingida";
  }

  function statusIcon(pos, meta){
  ...
  function render(month){
    ...
    tbodyDet.innerHTML = kpiRows.map(([nome, val, meta]) => `
      <tr>
        <td>${nome}</td>
        <td>${val.toFixed(1)}%</td>
        <td>${meta}%</td>
        <td>${statusTxt(val, meta)}</td>
      </tr>
    `).join("");
    ...
    insights.push(`<b>Privacidade:</b> este dashboard exibe apenas indicadores agregados. Evite publicar dados pessoais em repositório público.`);

    ul.innerHTML = insights.map(x => `<li>${x}</li>`).join("");

    // Charts
    const counts = [1,2,3,4,5].map(n => arr.filter(r => Number(r[FIELDS.nota]) === n).length);
    document.getElementById("satHint").textContent = `Média: ${k.mean.toFixed(2)}/5 • CSAT: ${k.csat.toFixed(1)}%`;
...
def main():
    ...
    # regra do cliente: Recusou => Sim + nota 5
    df_raw = df.copy()
    ...
    html = html.replace("__LOGOS_HTML__", logos_html)
    html = html.replace("__GENERATED__", generated)
    html = html.replace("__FOOTER__", FOOTER_TEXT)

    html = html.replace("__COL_ITENS__", COL_ITENS)
    html = html.replace("__COL_UNIFORME__", COL_UNIFORME)
    html = html.replace("__COL_PRODUTOS__", COL_PRODUTOS)
    html = html.replace("__COL_ATENDIMENTO__", COL_ATENDIMENTO)
    html = html.replace("__COL_HORARIO__", COL_HORARIO)
    html = html.replace("__COL_NOTA__", COL_NOTA)
